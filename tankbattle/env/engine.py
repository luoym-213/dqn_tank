...
class TankBattle(object):  # 坦克大战类
    ...
    def __init__(self, render=False, speed=60, max_frames=100000, frame_skip=1,
        ...
        # self.enemyA = enemyA  # 敌人A
        # self.enemyB = enemyB  # 敌人B
        ...
        self.sprites = pygame.sprite.Group()  # 小精灵
        self.enemies = pygame.sprite.Group()  # 敌人
        ...
        self.next_rewards_p1 = cl.deque(maxlen=100)  # 队列
        self.next_rewards_p2 = cl.deque(maxlen=100)
        self.num_of_objs = 2
        self.mytank_actions = GlobalConstants.MYTANK_ACTIONS
        # self.enemytank_actions=GlobalConstants.ENEMYTANK_ACTIONS
        ...
        # self.__generate_enemyA(self.enemyA)
        # self.__generate_enemyB(self.enemyB)
    ...
    def __enemies_update(self, num_of_enemies):  # 敌军更新
        ...
        #             for enemy in enumerate(self.enemies):
        #                 if enemy == self.enemies[0]:  # 敌方坦克0
        #                     rand_action = 0  # 左
        #                     if rand_action != GlobalConstants.FIRE_ACTION:
        #                         enemy.move(rand_action, self.sprites)
        #                     else:
        #                         enemy.fire_started_time = self.frames_count
        #                 elif enemy == self.enemies[1]:  # 敌方坦克1
        #                     rand_action = 1  # 右
        #                     if rand_action != GlobalConstants.FIRE_ACTION:
        #                         enemy.move(rand_action, self.sprites)
        #                     else:
        #                         enemy.fire_started_time = self.frames_count
        #                 elif enemy == self.enemies[2]:  # 敌方坦克2
        #                     rand_action = 2  # 上
        #                     if rand_action != GlobalConstants.FIRE_ACTION:
        #                         enemy.move(rand_action, self.sprites)
        #                     else:
        #                         enemy.fire_started_time = self.frames_count
        #                 elif enemy == self.enemies[3]:  # 敌方坦克3
        #                     rand_action = 3  # 下
        #                     if rand_action != GlobalConstants.FIRE_ACTION:
        #                         enemy.move(rand_action, self.sprites)
        #                     else:
        #                         enemy.fire_started_time = self.frames_count
        #                 elif enemy == self.enemies[4]:  # 敌方坦克4
        #                     rand_action = 0  # 左
        #                     if rand_action != GlobalConstants.FIRE_ACTION:
        #                         enemy.move(rand_action, self.sprites)
        #                     else:
        #                         enemy.fire_started_time = self.frames_count
        #             if not enemy.move(rand_action, self.sprites):#如果有障碍物
        #                 rand_action = np.random.randint(0, self.num_of_actions)
        #                 if rand_action != GlobalConstants.FIRE_ACTION:
        #                     enemy.move(rand_action, self.sprites)
        #                 else:
    ...
    def __render(self):  # 渲染
        ...
        self.__enemies_update(self.num_of_enemies)
    ...
    def step(self, action, action_p2=-1):
        ...
        return self.get_state(), self.__check_reward(), self.is_terminal(), None
    ...
    def get_state(self):
        pygame.pixelcopy.surface_to_array(self.current_buffer, self.screen)  # 将surface像素复制到数组对象
    ...
    def get_mytank_actions(self):
        return self.mytank_actions
